...
fs.readFile("data.txt", "utf8", (err, data) => {
	...
	const commonDivisible = allDivisibles.reduce((acc, item) => acc * item, 1);
	console.log(commonDivisible);
	...
	for (let round = 0; round < 10000; round++) {
		...
		for (let monkey = 0; monkey < howManyMonkeys; monkey++) {
			...
			for (let item = 0; item < initialLength; item++) {
				...
				itemModified = itemModified % commonDivisible;
